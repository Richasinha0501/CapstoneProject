		...
		console.log(this.email.valid );
		  console.log(this.email.value );
			  if(this.username.valid===false){
				alert('User Name should be in coorect format');
				console.log('User Name sholuld be in coorect format');
			  }
			  else  if(this.password.valid===false){
				alert('Password should be in coorect format');
				console.log('Password sholuld be in coorect format');
			  }
			  else  if(this.email.valid===false){
				alert('Email id should be in coorect format');
				console.log('Email id should be in correct format');
			  }
			  else {
		  this.message='CONGRATULATIONS! YOUR ACCOUNT IS SUCCESSFULLY CREATED :';
	       this.toast.success({detail:"CONGRATULATIONS! YOUR ACCOUNT IS CREATED", summary:"",duration:5000})
		  let re=this.service.addUser(this.user1);
		  re.subscribe((data)=>{this.message=data;
			console.log(data);
			...
			this.toast.success({detail:"REGISTERED!!!", summary:"",duration:4000})
			this.router.navigate(['login']);
		}
		  }
		  );
